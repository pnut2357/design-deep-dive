...
"""
    ...
    More information about consistent hashing can be read in these articles:
        ...
        Distributed caching protocols for relieving hot spots on the WWW":
...
class MemcacheRing(memcache.Client):
    ...
    def _get_server(self, key):
        if isinstance(type(key), types.TupleType):
...
def test_consistent_hashing():
    ...
    ring = HashRing(memcache_servers)
    requested_servers = [
        ring.get_node(f'my_key-{i+1}') for i in range(len(memcache_servers))
    ]

    print("+-----------------------------------------------------------+")
    print("| Test Consistent Hashing                                   |")
    print("+-----------------------------------------------------------+")
    print("|")
    print("+---------------------------------------+")
    print("| ring                                  |")
    ...
    print(f"|   - sorted_keys: {ring._sorted_keys}")
    # print(f"|   - nodes      : {ring.ring}")
    # print(f"|   - weights    : {ring.weights}")
    print("|")

    print("+---------------------------------------+")
    print("| requested servers                     |")
    print("+---------------------------------------+")
    for i, server in enumerate(requested_servers):
        print(f"  - {i+1} requested server: {server}")
    print()
